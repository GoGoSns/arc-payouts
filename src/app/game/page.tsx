...
const SEED_LB = [
  { name: 'WorlldZ',    score: 38, usdc: 19   },
  { name: 'ArcBuilder', score: 31, usdc: 15.5 },
  { name: 'CryptoKing', score: 24, usdc: 12   },
  { name: 'GoGo',       score: 18, usdc: 9    },
]
...
export default function GamePage() {
  ...
  const [leaderboard, setLeaderboard] = useState(SEED_LB)
  const [sending, setSending] = useState(false)
  const [txHash, setTxHash] = useState('')
  const [sendTarget, setSendTarget] = useState('')

  const stateRef = useRef({ score:0, usdc:0, combo:1, lives:3, best:0, gameState:'idle' as 'idle'|'running'|'over', shieldActive:false, slowActive:false, doubleActive:false, diff:'easy' as 'easy'|'medium'|'hard' })
  ...
  const loseLife = () => {
    ...
    if (newLives <= 0) {
      ...
      if (s.usdc > 0) {
        setLeaderboard(prev => {
          const next = [...prev, { name:'You', score:s.score, usdc:s.usdc }]
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      g.pipes?.forEach((p:any) => {
        ...
        ctx.fillRect(p.x,p.topH+gameRef.current.cfg.gap,28,H-p.topH-gameRef.current.cfg.gap)
        ctx.fillStyle=bc
        ctx.fillRect(p.x-3,p.topH-9,34,9)
        ctx.fillRect(p.x-3,p.topH+gameRef.current.cfg.gap,34,9)
        // waves on pipe
        ...
        <div style={{ background:'#0e0e0e', border:'1px solid #1a1a1a', borderRadius:14, padding:14 }}>
          ...
          {leaderboard.map((r, i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'7px 0', borderBottom: i<leaderboard.length-1?'1px solid #141414':'none' }}>
              ...
              <div style={{ fontSize:11, color:'#555', marginLeft:6 }}>= {typeof r.usdc==='number'?r.usdc.toFixed(2):r.usdc} USDC</div>
            </div>
          ))}
        </div>
